...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    @objc dynamic open var placeholderFont: UIFont? = UIFont(name: "SFProText-SemiboldItalic", size: 15.0) {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    @objc dynamic open var titleFont: UIFont = UIFont(name: "SFProText-Semibold", size: 13.0) ?? .systemFont(ofSize: 13) {
        didSet {
            updateTitleLabel()
        }
    }
    ...
    @IBInspectable dynamic open var titleColor: UIColor = UIColor("#757575") {
        didSet {
            updateTitleColor()
        }
    }
    ...
    @IBInspectable dynamic open var lineColor: UIColor = UIColor("#CBCBCB") {
        didSet {
            updateLineView()
        }
    }
    ...
    @IBInspectable dynamic open var disabledColor: UIColor = UIColor(white: 0.75, alpha: 1.0) {
        didSet {
            updateControl()
            updatePlaceholder()
        }
    ...
    */
    ...
    fileprivate func updateTitleLabel(_ animated: Bool = false) {
        ...
        let ranges = titleText?.ranges(of: "*").count ?? 0
        titleLabel.halfText(changeText: "*", color: .red, location: ranges <= 1 ? 0 : ranges - 1)
        titleLabel.font = titleFont
    ...
    */
    ...
    func placeholderOrTitle() -> String? {
        guard let placeholder = placeholder ?? title else {
            return nil
        }
        return placeholder
    }
    ...
    //MARK:- Show Shake Error Label
    public func shakeError(_ msg: String) {
